...
import { get_all_events } from '~/composables/events';
...
<template>
                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" method="post">
                        <div>
                            ...
                            <input v-model="event_name" type="text" name="event_name" id="event_name"
                                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                placeholder="予定の名前を入力" autocomplete="off">
                        </div>
                        <div>
                            <label for="start_time" class="block mb-2 text-sm font-medium text-gray-900">開始日時</label>
                            <input v-model="start_time" type="datetime-local" name="start_time" id="start_time"
                                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                autocomplete="off">
                        </div>
                        <div>
                            <label for="end_time" class="block mb-2 text-sm font-medium text-gray-900">終了日時</label>
                            <input v-model="end_time" type="datetime-local" name="end_time" id="end_time"
                                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                autocomplete="off">
                        </div>
                        <div>
                            <label for="memo" class="block mb-2 text-sm font-medium text-gray-900">メモ・備考</label>
                            <textarea v-model="memo" type="text" name="memo" id="memo" placeholder="備考を入力"
                                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        ...
                        <button @click="create($event)" type="submit"
                            class="flex items-center mx-auto justify-center w-1/2 font-medium text-sm px-5 py-2.5 text-center bg-blue-600 text-white hover:bg-blue-800 rounded-lg">
                            送信
                        </button>
                    </form>
                </div>
    ...
    <section>
        <button @click="get_all($event)">test</button>
    </section>
</template>
