                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" method="post">
                        <div>
                            ...
                            <input v-model="email" :class="{ 'form_val_error': email_val === 'form_val_error' }"
                                type="email" name="email" id="email"
                                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                placeholder="有効なメールアドレスを入力" autocomplete="username">
                            <p class="text-xs">{{ email_error_msg }}</p>
                        </div>
                        <div>
                            <label for="password" class="block mb-2 text-sm font-medium text-gray-900">パスワード</label>
                            <input v-model="password" :class="{ 'form_val_error': password_val === 'form_val_error' }"
                        ...
                        <button @click="loginUser($event)" type="submit"
                        ...
                        <p class="text-sm font-light text-gray-500">
                            TaskWithアカウントをお持ちではない場合: <nuxt-link to="/signUp"
                                class="font-medium text-primary-600 hover:underline">アカウントを作成</nuxt-link>
                        </p>
                    </form>
                </div>
            </div>
