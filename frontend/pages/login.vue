...
<template>
    <section>
        <div class="flex flex-col items-center justify-center px-4 py-8 mx-auto lg:py-0" style="height: 100svh;">
            ...
            <div class="w-full max-w-xs md:max-w-md bg-white rounded-lg shadow">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" method="post">
                        <div>
                            ...
                            <input v-model="email" :class="{ 'form_val_error': email_val === 'form_val_error' }" type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="有効なメールアドレスを入力" autocomplete="username">
                            <p class="text-xs">{{ email_error_msg }}</p>
                        </div>
                        <div>
                            <label for="password" class="block mb-2 text-sm font-medium text-gray-900">パスワード</label>
                            <input v-model="password" :class="{ 'form_val_error': password_val === 'form_val_error' }" type="password" name="password" id="password" placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" autocomplete="current-password">
                            <p class="text-xs">{{ pass_error_msg }}</p>
                        </div>
                        <div class="flex items-center justify-between">
                            <div class="flex items-start">
                            </div>
                            <a href="#" class="text-sm font-medium text-primary-600 hover:underline">パスワードを忘れた場合</a>
                        </div>
                        <button @click="loginUser($event)" type="submit" class="w-full bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                            ログイン
                        </button>
                        <p class="text-sm font-light text-gray-500">
                            TaskWithアカウントをお持ちではない場合: <nuxt-link to="/signUp" class="font-medium text-primary-600 hover:underline">アカウントを作成</nuxt-link>
                        </p>
                    </form>
                </div>
            </div>
